...
On supported interactive Unix terminals, the managed Codex TUI displays `codex-switch <version>` right-aligned in the top-right corner. codex-switch relays Codex through a PTY and composes the label at synchronized frame boundaries in both the main inline conversation and alternate-screen views; it does not require a modified Codex binary. The existing direct execution path is used without the label when any standard stream is redirected, `--no-alt-screen` is set, the outer terminal is already raw or reports `TERM=dumb`, or terminal sizing is unavailable. If a relayed Codex session does not emit the recognized synchronized frames, its bytes remain unchanged and no label is added. Terminals that are too narrow for the complete label suppress it until it fits.
