...
On supported interactive Unix terminals, the managed Codex TUI displays `codex-switch <version>` right-aligned in the top-right corner. codex-switch relays Codex through a PTY and composes the label at synchronized alternate-screen frame boundaries; it does not require a modified Codex binary. The existing direct execution path is used without the label when any standard stream is redirected, `--no-alt-screen` is set, the outer terminal is already raw or reports `TERM=dumb`, terminal sizing is unavailable, or Codex does not emit the recognized alternate-screen frames. Terminals that are too narrow for the complete label suppress it until it fits.
